...
function buildNarrativeMarkdown(item, evState) {
  const lines = [];
  ...
  lines.push("## Evidence Checklist");
  lines.push("");
  const completed = (item.evidence_checklist || []).filter((e, idx) => evState[idx]).length;
  const total = (item.evidence_checklist || []).length;
  lines.push(`**Progress:** ${completed}/${total} items complete`);
  lines.push("");
  (item.evidence_checklist || []).forEach((e, idx) => {
    const mark = evState[idx] ? "✓" : "○";
    lines.push(`${mark} ${e}`);
  });
  lines.push("");
...
<h3>How VA rates it (high-level)</h3>
    ...
    <button id="pkt2DownloadPdf" class="miniBtn" type="button">Download PDF Packet</button>

    <div style="display:flex; gap:14px; margin-top:8px; font-size:12px; flex-wrap:wrap;">
      <label><input type="checkbox" id="pkt2SepPages" checked /> Separator pages</label>
      <label><input type="checkbox" id="pkt2InclSnips" checked /> Include snippets</label>
      <label><input type="checkbox" id="pkt2SummaryPages" checked /> Summary page per exhibit</label>
      <label><input type="checkbox" id="pkt2SummarySnips" checked /> Summary snippet preview</label>
    </div>

    <div style="display:flex; gap:14px; margin-top:8px; font-size:12px; flex-wrap:wrap;">
      <label><input type="checkbox" id="pkt2EdgeJustify" checked /> Save edge justification</label>
      <label><input type="checkbox" id="pkt2NodeNotes" checked /> Also add notes to both evidence nodes</label>
    </div>

    <div id="pkt2BundleUI" style="margin-top:10px;"></div>

    <hr style="margin:12px 0;" />

    <div style="display:flex; gap:8px; align-items:center; justify-content:space-between;">
      <strong style="font-size:12px;">Narrative v2 (Cited)</strong>
      <button id="nar2GenBtn" type="button">Generate Cited Narrative</button>
    </div>

    <div style="display:flex; gap:8px; margin-top:8px;">
      <button id="nar2CopyBtn" type="button" disabled>Copy</button>
    </div>

    <textarea id="nar2Out" style="width:100%; height:220px; margin-top:8px;" placeholder="Cited narrative will appear here..."></textarea>

    <hr style="margin:12px 0;" />

    <div style="display:flex; gap:8px; align-items:center; justify-content:space-between;">
      <strong style="font-size:12px;">Narrative v3 (DC-aware Strategy)</strong>
      <button id="nar3GenBtn" type="button">Generate DC Strategy</button>
    </div>

    <div style="display:flex; gap:8px; margin-top:8px;">
      <button id="nar3CopyBtn" type="button" disabled>Copy</button>
    </div>

    <textarea id="nar3Out" style="width:100%; height:260px; margin-top:8px;" placeholder="Narrative v3 output will appear here..."></textarea>
...
`).join("")}
  ...
  // PDF Packet download
  const pkt2DownloadPdfBtn = document.getElementById("pkt2DownloadPdf");
  if (pkt2DownloadPdfBtn) {
    pkt2DownloadPdfBtn.addEventListener("click", async () => {
      try {
        pkt2DownloadPdfBtn.disabled = true;
        pkt2DownloadPdfBtn.textContent = "Generating PDF...";

        const st = loadEvidenceState(item.id);
        
        // Prepare PDF options from checkboxes
        const pdfOptions = {
          ...
          summaryPages: document.getElementById("pkt2SummaryPages")?.checked !== false,
          summarySnippetPreview: document.getElementById("pkt2SummarySnips")?.checked !== false
        };

        // Build bundle similar to buildClaimPacketText but with structured data
        const bundle = {
          condition: {
            name: item.name,
            id: item.id
          ...
          readiness: {
            score: 0, // You can calculate this based on evidence state
            cfr: {
              cfrRefs: item.cfr?.map(c => c.section) || [],
              dcs: item.cfr?.map(c => c.diagnostic_code) || []
            }
          },
          exhibits: [],
          narrativeMarkdown: buildNarrativeMarkdown(item, st),
          pdfOptions: pdfOptions
        };

        // Add exhibits from evidence links
        const links = loadEvidenceLinks(item.id);
        links.forEach((link, idx) => {
          bundle.exhibits.push({
            exhibitId: `EX-${String(idx + 1).padStart(3, '0')}`,
            evidenceId: link.id || `ev-${idx}`,
            title: link.label || "Untitled",
        ...
        const response = await fetch("/api/packet-v2-pdf", {
          method: "POST",
          headers: {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = `${item.id}_claim_packet.pdf`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);

        // NEW: Store bundle and render UI
        window.__lastPacketV2 = bundle;
        renderPkt2BundleUI(bundle);

      } catch (error) {
        console.error("PDF generation error:", error);
        alert(`Failed to generate PDF: ${error.message}`);
      } finally {
        pkt2DownloadPdfBtn.disabled = false;
        pkt2DownloadPdfBtn.textContent = "Download PDF Packet";
      }
    });
  }

  // Narrative V2 wiring
  const nar2GenBtn = document.getElementById("nar2GenBtn");
  const nar2CopyBtn = document.getElementById("nar2CopyBtn");

  if (nar2GenBtn) {
    nar2GenBtn.addEventListener("click", nar2Generate);
  }

  if (nar2CopyBtn) {
    nar2CopyBtn.addEventListener("click", nar2Copy);
  }

  // Narrative V3 wiring
  const nar3GenBtn = document.getElementById("nar3GenBtn");
  const nar3CopyBtn = document.getElementById("nar3CopyBtn");

  if (nar3GenBtn) {
    nar3GenBtn.addEventListener("click", nar3Generate);
  }

  if (nar3CopyBtn) {
    nar3CopyBtn.addEventListener("click", nar3Copy);
  }
...
// -------- Packet V2 State Helpers --------

function nowIso() { return new Date().toISOString(); }

function ensureArray(v) { return Array.isArray(v) ? v : (v ? [v] : []); }

function getStateStorageKeyGuess() {
  // If you already have a known key, replace this with it.
  if (localStorage.getItem("vaCfrFinderState")) return "vaCfrFinderState";
  if (localStorage.getItem("VA_CFR_FINDER_STATE")) return "VA_CFR_FINDER_STATE";
  return "vaCfrFinderState"; // sensible default
}

function saveAppState(state) {
  const key = getStateStorageKeyGuess();
  localStorage.setItem(key, JSON.stringify(state));
}

function getEvidenceArrayRef(state) {
  // normalize possible shapes
  if (Array.isArray(state.evidence)) return { key: "evidence", arr: state.evidence };
  if (Array.isArray(state.evidenceNodes)) return { key: "evidenceNodes", arr: state.evidenceNodes };
  // create if missing
  state.evidence = [];
  return { key: "evidence", arr: state.evidence };
}

function getEdgesArrayRef(state) {
  if (Array.isArray(state.edges)) return { key: "edges", arr: state.edges };
  if (Array.isArray(state.relationships)) return { key: "relationships", arr: state.relationships };
  state.edges = [];
  return { key: "edges", arr: state.edges };
}

function findEvidenceById(state, evidenceId) {
  const { arr } = getEvidenceArrayRef(state);
  const id = String(evidenceId);
  return arr.find(ev => String(ev.id || ev._id || ev.uuid || ev.key || ev.evidenceId) === id) || null;
}

function appendEvidenceNote(ev, noteText, meta = {}) {
  if (!ev) return;
  if (!Array.isArray(ev.notes)) ev.notes = [];
  ev.notes.push({
...
function edgeExists(edgesArr, edge) {
  return edgesArr.some(e =>
    (String(e.from) === String(edge.from) && String(e.to) === String(edge.to) && (e.type || "") === (edge.type || "")) ||
    (String(e.from) === String(edge.to) && String(e.to) === String(edge.from) && (e.type || "") === (edge.type || ""))
  );
}

// -------- Packet V2 Bundle UI Functions --------

function renderPkt2BundleUI(bundle) {
  const host = document.getElementById("pkt2BundleUI");
  if (!host) return;

  host.innerHTML = "";
  if (!bundle?.exhibits?.length) {
    host.innerHTML = `<div style="font-size:12px; opacity:.85;">No exhibits in packet.</div>`;
    return;
  }
  ...
  bundle.exhibits.forEach(ex => {
    ...
    top.innerHTML = `
      <div><strong>${ex.exhibitId}</strong> • ${ex.roleLabel || "General"}${(ex.roleConfidence != null) ? ` (conf ${ex.roleConfidence})` : ""}</div>
      <div style="opacity:.85; margin-top:2px;"><strong>${ex.title}</strong></div>
      <div style="opacity:.75; margin-top:2px;">${ex.type || "unknown"}${ex.date ? " • " + ex.date : ""} • connections: ${ex.edgeCount ?? 0}</div>
    `;
    card.appendChild(top);

    // Neighbors
    const neigh = document.createElement("div");
    neigh.style.marginTop = "8px";
    neigh.style.fontSize = "12px";
    const neighbors = ex.neighbors || [];
    neigh.innerHTML = `<strong>Neighbors:</strong> ${neighbors.length ? "" : "<span style='opacity:.75'>(none)</span>"}`;
    card.appendChild(neigh);
    ...
    if (neighbors.length) {
      ...
      neighbors.slice(0, 8).forEach(n => {
        ...
        li.style.opacity = ".9";
        li.textContent = `${n.title}${n.type ? " (" + n.type + ")" : ""}`;
        ul.appendChild(li);
      });
      card.appendChild(ul);
    }
    ...
    recBox.style.fontSize = "12px";
    recBox.innerHTML = `<strong>Recommended links:</strong>`;
    card.appendChild(recBox);

    const recs = ex.recommendedLinks || [];
    if (!recs.length) {
      const none = document.createElement("div");
      none.style.opacity = ".75";
      none.style.marginTop = "4px";
      none.textContent = "None (already well-connected or no good targets).";
      ...
      recs.slice(0, 4).forEach(r => {
        ...
        const toRef = r.toExhibitId ? `${r.toExhibitId}` : "(not in packet)";
        label.innerHTML = `<strong>${r.type}</strong> → <strong>${toRef}</strong> • ${r.toTitle}`;
        row.appendChild(label);

        const reason = document.createElement("div");
        reason.style.opacity = ".8";
        ...
        apply.type = "button";
        apply.textContent = "Apply Edge";
        apply.addEventListener("click", async () => {
          await applyPacketLinkAndRecompute(r);
        });
        btnRow.appendChild(apply);

        const copy = document.createElement("button");
        copy.type = "button";
        copy.textContent = "Copy Edge JSON";
        copy.addEventListener("click", async () => {
          const edge = { from: r.fromEvidenceId, to: r.toEvidenceId, type: r.type };
          await navigator.clipboard.writeText(JSON.stringify(edge, null, 2));
        });
...
async function applyPacketLinkAndRecompute(r) {
  const saveJust = document.getElementById("pkt2EdgeJustify")?.checked !== false;
  const addNodeNotes = document.getElementById("pkt2NodeNotes")?.checked !== false;

  const edge = {
    from: r.fromEvidenceId,
    to: r.toEvidenceId,
    type: r.type,
    // NEW metadata (safe)
    source: "packet-v2",
    createdAt: nowIso(),
    updatedAt: nowIso(),
    ...(saveJust ? { justification: r.reason || "" } : {})
  };

  // Preferred: use your existing edge-apply function if it exists
  // But we also persist + optionally add node notes.
  const state = getAppState?.();
  if (!state) return;

  // 1) Add/merge edge into state (single source of truth)
  const { arr: edgesArr } = getEdgesArrayRef(state);
  if (!edgeExists(edgesArr, edge)) {
    edgesArr.push(edge);
  } else {
    // If exists, update justification if missing
    const existing = edgesArr.find(e =>
      (String(e.from) === String(edge.from) && String(e.to) === String(edge.to) && (e.type || "") === (edge.type || "")) ||
      (String(e.from) === String(edge.to) && String(e.to) === String(edge.from) && (e.type || "") === (edge.type || ""))
    );
    if (existing) {
      existing.updatedAt = nowIso();
      if (saveJust && !existing.justification && edge.justification) existing.justification = edge.justification;
      if (!existing.source) existing.source = "packet-v2";
    }
  }

  // 2) Add notes to evidence nodes (optional)
  if (addNodeNotes) {
    const fromEv = findEvidenceById(state, edge.from);
    const toEv = findEvidenceById(state, edge.to);

    const note = saveJust && edge.justification
      ? `Linked via ${edge.type}: ${edge.justification}`
      : `Linked via ${edge.type} (packet-v2 recommendation).`;

    appendEvidenceNote(fromEv, note, { kind: "edge_justification", edgeType: edge.type, otherEvidenceId: edge.to });
    appendEvidenceNote(toEv, note, { kind: "edge_justification", edgeType: edge.type, otherEvidenceId: edge.from });
  }

  // 3) Persist state
  saveAppState(state);

  // 4) If your graph UI has a live edge-add method, call it to refresh visuals
  try {
    if (typeof applySuggestedEdge === "function") {
      // Some versions of your applySuggestedEdge will also update state; ours already did.
      applySuggestedEdge({ from: edge.from, to: edge.to, type: edge.type });
    } else if (typeof window.addEdge === "function") {
      window.addEdge({ from: edge.from, to: edge.to, type: edge.type });
    }
  } catch (e) {
    console.warn("Graph UI edge add failed (state still saved):", e);
  }

  // 5) Recompute loop: score → gaps → packet
  try {
    if (typeof refreshScore === "function") await refreshScore();
    if (typeof refreshGaps === "function") await refreshGaps();
    if (typeof pkt2Generate === "function") await pkt2Generate();
  } catch (e) {
    console.error("Recompute loop failed:", e);
  }
}

// -------- Narrative V2 Functions --------

async function nar2Generate() {
  const state = getAppState?.();
  if (!state) {
  ...
  const btn = document.getElementById("nar2GenBtn");
  btn.disabled = true;
  const old = btn.textContent;
  btn.textContent = "Generating...";

  try {
    // Use current Packet v2 selection if present
    const selectedEvidenceIds = (typeof getSelectedEvidenceIdsFromUI === "function")
      ? getSelectedEvidenceIdsFromUI()
      : [];

    const response = await fetch("/api/intelligence/narrative-v2", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ state, conditionId, selectedEvidenceIds })
    });
    ...
    document.getElementById("nar2Out").value = out.narrativeMarkdown || "";
    document.getElementById("nar2CopyBtn").disabled = !out.narrativeMarkdown;

    // optional: store
    window.__lastNarrativeV2 = out;
    console.log("[NarrativeV2]", out);
  } catch (e) {
    document.getElementById("nar2Out").value = `ERROR: ${e.message}`;
    document.getElementById("nar2CopyBtn").disabled = true;
    console.error("Narrative V2 generation failed:", e);
  } finally {
    btn.disabled = false;
    btn.textContent = old;
  }
...
async function nar2Copy() {
  const txt = document.getElementById("nar2Out").value || "";
  if (!txt) return;
  await navigator.clipboard.writeText(txt);
  alert("Copied!");
}

// -------- Narrative V3 Functions --------

async function nar3Generate() {
  const state = getAppState?.();
  if (!state) {
    alert("No state available");
    return;
  ...
  const btn = document.getElementById("nar3GenBtn");
  btn.disabled = true;
  const old = btn.textContent;
  btn.textContent = "Generating...";
  ...
  try {
    ...
    const response = await fetch("/api/intelligence/narrative-v3", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ state, conditionId, selectedEvidenceIds })
    });
    ...
    document.getElementById("nar3Out").value = out.narrativeMarkdown || "";
    document.getElementById("nar3CopyBtn").disabled = !out.narrativeMarkdown;

    window.__lastNarrativeV3 = out;
    console.log("[NarrativeV3]", out);
  } catch (e) {
    document.getElementById("nar3Out").value = `ERROR: ${e.message}`;
    document.getElementById("nar3CopyBtn").disabled = true;
    console.error("Narrative V3 generation failed:", e);
  } finally {
    btn.disabled = false;
    btn.textContent = old;
  }
...
async function nar3Copy() {
  const txt = document.getElementById("nar3Out").value || "";
  if (!txt) return;
  await navigator.clipboard.writeText(txt);
  alert("Copied!");
}
